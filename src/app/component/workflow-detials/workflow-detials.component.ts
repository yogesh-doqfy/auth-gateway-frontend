...
@Component({
  selector: 'app-workflow-detials',
  templateUrl: './workflow-detials.component.html',
  styleUrls: ['./workflow-detials.component.scss', "./mobile.scss"]
})
export class WorkflowDetialsComponent implements OnInit {
...
isChecked = true;
run = false;
constructor(private wfService: WebflowService, private route: ActivatedRoute, private messageService: MessagesService, private userService:UserService) { }
...
ngOnInit(): void {
  ...
  this.run = this.route.snapshot.data['run']
  this.run = true
}
  ...
  closeModal(){
    this.isChecked = this.api.is_subscribed
      ...
      handleSwitchChange(api_id:number): void {
        ...
        this.wfService.subscribeToWF(api_id).subscribe(
          (response) => {
            console.log(response)
            this.api.is_subscribed = !this.api.is_subscribed
            this.isChecked = this.api.is_subscribed
